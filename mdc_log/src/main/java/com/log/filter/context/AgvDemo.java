...
import static com.log.filter.formatter.CmdLogFormatter.formatter;
import static com.log.filter.log.LogUtils.info;
 ...
 */
@Component
public class AgvDemo implements Runnable{
    @PostConstruct
    public void init(){
    ...
    public void run() {
        while (true){
            ...
            info(formatter(name,"agv run","agv text"));
            try {
                Thread.sleep(1000);
